...
class CompanyRepo implements CompanyInterface 
{
	...
	public function getTemplate($comp_id){
			...
			$getcosttemplate=\DB::table('cost_templates as ct')
								  ...
								  ->select('project_id','field_id','operator_id','production','l.level','ct.level as level_id','status','review','license','ct.created_at','ct.id','ct.reason')
    ...
    public function getoperator($operatorid){
        ...
        return \DB::table('operators')
                    ->where('id',$operatorid)
                    ->select('company as name','contact_email as email','contact_firstname as firstname','contact_lastname as lastname')
                    ->first();
 
    }

    public function resolve_decision($decision){
        if($decision==1){
            return 'Accepted';
        }
        if($decision==2){

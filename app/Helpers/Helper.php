...
class helper {
    ...
    public static function resolve_terrain($fieldid,$type){
        ...
        if($type==3){
            $blockid=\App\field::where('id',$fieldid)->select('block_id')->value('block_id');
            $biz_arrange= \App\block::where('id',$blockid)->select('lease_id')->value('lease_id');
    ...
    public static function resolve_Projectname($projectid){
    ...
    public static function getdev(){
        ...
        return \App\cost_setting::where('id',1)->select('percent_dev')->value('percent_dev');
